...
async def get_random_duck_gif():
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get("https://random-d.uk/api/v2/random?type=gif&format=json") as resp:
                if resp.status == 200:
                    data = await resp.json()
                    return data['url']
    except Exception as e:
        print(f"Ördek GIF'i alınırken hata oluştu: {e}")
    return None
...
async def send_random_media():
    # Randomly choose between cat and duck
    animal_type = random.choice(['kedi', 'ördek'])
    ...
    if animal_type == 'kedi':
        ...
        gif_url = await get_random_duck_gif()
        title = "🦆 Günün Ördek GIF'i"
        description = "Vak vak! Bu ördek seni çok sevdi! 🦆❤️"
...
async def send_daily_random_thing():
    try:
        with open('random_messages.json', 'r', encoding='utf-8') as f:
            messages = json.load(f)
        
        message = random.choice(messages)
        
        embed = discord.Embed(
            title="🎁 Günün Küçük Sürprizi",
            description=message,
            color=discord.Color.gold(),
            timestamp=datetime.now()
        )
        embed.set_footer(text="Seni hep düşünüyorum... ❤️")
        ...
        print(f"Günlük sürpriz mesaj gönderilirken hata oluştu: {e}")
...
async def on_ready():
    ...
    # Daily random sweet message at 10:30
    scheduler.add_job(send_daily_random_thing, CronTrigger(hour=10, minute=30))
    ...
    # Random GIFs (Cat or Duck - 3 times daily)
    gif_times = ["11:25", "14:30", "19:30"]
    for t in gif_times:
        h, m = map(int, t.split(':'))
        scheduler.add_job(send_random_media, CronTrigger(hour=h, minute=m))
